...
- **禁止推销语气**：不使用"直击痛点"、"核心卖点"、"为什么值得关注"、"值得一试"、"适合人群"等营销话术
- **禁止拉踩竞品**：不做产品对比表格来突出某工具的优势，如需对比应客观中立
- **禁止过度修辞**：不用"最有意思的"、"非常强大"、"令人惊叹"等主观评价
- **禁止多余总结段**：如果正文已经讲清楚了，"总结"段不应只是把前文重说一遍；有价值的总结应提炼新观点或给出下一步行动建议
- **禁止无意义的 emoji**：不用 ✅ ❌ ⚠️  等 emoji 装饰正文。技术文章用文字表达即可，emoji 仅可用于代码块注释或特殊标记场景

#### 文章开头规范

- 直接进入主题，用 1-2 句话说明本文要解决什么问题或介绍什么内容
- 不要用"XX 已经不稀奇了"、"但有一个痛点始终存在"之类的铺垫
- 示例：

```markdown
## 解决 Astro 开发服务器无法通过 Tailscale 访问的问题

Astro 开发服务器默认绑定 `localhost`（127.0.0.1），只接受本机连接，
导致通过 Tailscale 或局域网 IP 无法访问。本文说明原因和解决方法。
```
