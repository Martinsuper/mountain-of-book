...
VoxCPM2 的质量优势和它的推理成本（2B 参数、8GB VRAM、RTF ~0.30）共同决定了它的适用边界：

- **擅长**：多语言 TTS（30 语言自动切换）、声音克隆产品（三种克隆模式覆盖不同精度）、有声书/配音（48kHz 专业质量）、个性化语音定制（LoRA 微调，5-10 分钟音频即可适配）、声音创意设计（纯文本创建声音，无需录音）。
- **需权衡**：实时语音对话（标准推理 RTF ~0.30，需搭配 vLLM 加速）、低资源语言（部分如缅甸语、老挝语第三方评测表现一般）。
- **不适合**：低资源设备部署（2B + 8GB VRAM，不适合移动端或嵌入式）。
